...
public class ModelFromShapesBlockMakingTest {
    ...
    public void testBlockMaking() throws Exception {
        ...
        assertEquals(19.0, ((TelescopicPoint)p.get(1)).getPlanarY(), epsilon);
        assertEquals(-1.0, ((TelescopicPoint)p.get(2)).getPlanarX(), epsilon);
        assertEquals(20.0, ((TelescopicPoint)p.get(2)).getPlanarY(), epsilon);
        ...
        assertEquals(0, Point3D.computeDistance( new Point3D(1.0, 19.0,1.0), b.getCeilingPoints().get(1)), epsilon);
        assertEquals(0, Point3D.computeDistance( new Point3D(-1.0, 20.0,1.0), b.getCeilingPoints().get(2)), epsilon);
        ...
        assertEquals(0, Point3D.computeDistance( new Point3D(1.0, 19.0,0.0), b.getGroundPoints().get(1)), epsilon);
        assertEquals(0, Point3D.computeDistance( new Point3D(-1.0, 20.0,0.0), b.getGroundPoints().get(2)), epsilon);
